...
object Anagrams {
  ...
  /** Converts the word into its character occurrence list.
    ...
    */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy((c: Char) => c).toList.map(x => (x._1,x._2.length)).sortBy(x=>x._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if(occurrences.isEmpty)
      ...
      for{
        ...
        count <- 0 to occurrences.head._2
      } yield{
        if(count==0) combi
        else List((occurrences.head._1,count)) ::: combi
      }
...
//  def combinations(occurrences: Occurrences): List[Occurrences] =
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    ...
    def subFunc(XMap: Map[Char, Int], leftSide: (Char,Int)) = {
      val inX = XMap  getOrElse (leftSide._1,0)
      if(inX ==0) XMap
      else {
        if(inX == leftSide._2) XMap - leftSide._1
        else if (inX >= leftSide._2) XMap updated(leftSide._1, inX - leftSide._2)
        else XMap
      }
    }

    (y.toMap foldLeft x.toMap)(subFunc).toList
  }
